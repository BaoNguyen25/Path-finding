...
def ids(s, g, w, h, limited):
    limit = 100
    for depth in range(0, limit):
        visited, cost, path = dls(s, g, w, h, limited, depth)
        if visited and cost > 0:
            break
    return visited, cost, path


def dls(s, g, w, h, limited, depth):
    # Containers
    ...
    while stack:
        ...
        for i in range(len(dx)):
            ...
            if x in range(w) and y in range(h) and not visited[y][x] and distance[u.y][u.x] < depth:
                visited[y][x] = True
